...
# from pixel_to_world_trial import *

#ADDED - Rough Extrinsic
...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None, dh_config_file=None):
        ...
        # self.ui.btnUser3.clicked.connect(lambda: self.rxarm.gripper_state=True)
        self.ui.btnUser4.setText('Execute')
        self.ui.btnUser4.clicked.connect(partial(nxt_if_arm_init, 'execute'))
        self.ui.btnUser5.setText('Record Waypoint')                                     #ADDED
        self.ui.btnUser5.clicked.connect(partial(nxt_if_arm_init, 'capture_waypoint'))  #ADDED. Was USER 5
        self.ui.btnUser6.setText('Reset waypoints')                                     #ADDED
        self.ui.btnUser6.clicked.connect(partial(nxt_if_arm_init, 'reset_waypoints'))   #ADDED. Was USER 6

        self.ui.btnUser7.setText('Close/Get Gripper State')                                     #ADDED
        self.ui.btnUser7.clicked.connect(partial(nxt_if_arm_init, 'closegripper'))   #ADDED. Was USER 6

        self.ui.btnUser8.setText('Open/Get Gripper State')                                     #ADDED
        self.ui.btnUser8.clicked.connect(partial(nxt_if_arm_init, 'opengripper'))   #ADDED. Was USER 6

        #added. was button 9
        self.ui.btnUser9.setText('Start/stop Click2Move')
        self.ui.btnUser9.clicked.connect(partial(nxt_if_arm_init, 'click2move')) 
    ...
    def updateJointReadout(self, joints):
        for rdout, joint in zip(self.joint_readouts, joints):
            ...
            self.joint_record.append(joint)
        # print(self.joint_record)
        # np.savetxt('record.npy', self.joint_record)            
...
.getPerspectiveTransform( src_points, dst_points )
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            if self.camera.calibrate_flag == 0:
                world_x, world_y, world_z = pixel_to_world(pt.x(), pt.y(), z)
                # print(f'Coordinates = {world_x}, {world_y}, {world_z}')
                # print("1111111111111")
            else:   
                world_x, world_y, world_z = pixel_to_world_auto(pt.x(), pt.y(), self.camera.calibrate_flag, self.camera.H, self.camera.intrinsic, self.camera.extrinsic, self.camera.DepthFrameRaw)
                # world_x, world_y, world_z = pixel_to_world_trial(pt.x(), pt.y(), z, self.camera.extrinsic)
                # world_x, world_y, world_z = pixel_to_world(pt.x(), pt.y(), z)
                # print(f'Coordinates = {world_x}, {world_y}, {world_z}')
                # print("0000000000000000000")
                # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")

            # self.ui.rdoutMouseWorld.setText("(%s, %s, %s)"%(round(world_x[0],3), round(world_y[0],3), round(world_z[0],3)))
            self.ui.rdoutMouseWorld.setText("(%s, %s, %s)"%(round(world_x,3), round(world_y,3), round(world_z,3)))
